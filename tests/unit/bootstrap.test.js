...
test('Start New requires confirmation before replacing a current run', async (t) => {
  ...
  assert.deepEqual(controller.currentMatch, currentMatch)
  assert.notEqual(controller.currentMatch, currentMatch)
  assert.equal(app.resumeAvailable, true)
  await app.destroy()
})
...
test('declining Start New while restoration is pending preserves the saved run', async (t) => {
  ...
  assert.deepEqual(app.runSnapshot.match, savedMatch)
  assert.notEqual(app.runSnapshot.match, savedMatch)
  assert.equal(app.resumeAvailable, true)
