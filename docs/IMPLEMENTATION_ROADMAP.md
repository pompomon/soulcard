...
- **Acceptance evidence:** `src/presentation/input.js` owns primary mouse, touch, and pen
  ...
  and pending-restore races. An integration test proves one pointer action produces one
  deterministic clash, save, and presentation while renderer callbacks receive detached
  frozen state.
- **Validation:** All 231 unit/integration tests and the production build pass locally on
  ...
  focus outlines, and zero relevant console errors. A user-provided portrait screenshot
  from a physical Android target device confirmed touch navigation and responsive HUD
